...
public class ProgLang {
    ...
    private void addOrCreate(LinkedHashMap<String, LinkedHashSet<String>> programmersMap, String key, String value) {
        LinkedHashSet<String> element = programmersMap.computeIfAbsent(key, k -> new LinkedHashSet<String>());
        element.add(value);
    }
    ...
    public Map<String, LinkedHashSet<String>> getLangsMapSortedByNumOfProgs() {
        return this.sorted(this.getLangsMap(),
                Comparator.comparing((Map.Entry<String, LinkedHashSet<String>> entry) -> -entry.getValue().size())
                        .thenComparing(Map.Entry::getKey, String::compareTo));
    }

    public Map<String, LinkedHashSet<String>> getProgsMapSortedByNumOfLangs() {
        return this.sorted(this.getProgsMap(),
                Comparator.comparing((Map.Entry<String, LinkedHashSet<String>> entry) -> -entry.getValue().size())
                        .thenComparing(Map.Entry::getKey, String::compareTo));
    }
    ...
    public <T, U> Map<T, U> filtered(Map<T, U> map, Predicate<Map.Entry<T, U>> fn) {
        return map.entrySet().stream().filter(fn).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue,
